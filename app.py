...
st.set_page_config(page_title="Aryavarta Automation - Sales & Engineering Suite", page_icon="⚡", layout="wide")
...
def generate_pdf_quotation(client_name, panel_type, motor_kw, brand, bom_df, labor, margin, total_price, curr_sym="₹"):
    ...
    story = [
        ...
        Paragraph("<b>Manufacturer & Exporter of Switchgear Panels, VFD Panels & Automation Systems</b>", styles['Normal']),
        Paragraph("Gat No. 1610, Dehu Alandi Road, Chikhali, Pune - 411062 | GSTIN: 27ABOFA4930E1ZH", styles['Normal']),
        Spacer(1, 10)
    ]
    meta = [[Paragraph(f"<b>Client:</b> {client_name}", styles['Normal']), Paragraph(f"<b>Quote Ref:</b> AA/QT/{os.urandom(2).hex().upper()}", styles['Normal'])],
            [Paragraph(f"<b>Scope:</b> {panel_type} ({motor_kw})", styles['Normal']), Paragraph(f"<b>Date:</b> {pd.Timestamp.now().strftime('%d-%b-%Y')}", styles['Normal'])]]
    t_meta = Table(meta, colWidths=[270, 270]); t_meta.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#F7FAFC")), ('BOX', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 6)]))
    story.extend([t_meta, Spacer(1, 15), Paragraph("<b>Bill of Materials & Technical Specifications</b>", styles['Heading3'])])
    
    table_data = [["Item", "Brand", "Qty", f"Unit ({curr_sym})", f"Total ({curr_sym})"]]
    for idx, r in bom_df.iterrows():
        table_data.append([f"{r['Item']} ({r['Specification']})", str(r['Brand']), str(r['Qty']), f"{curr_sym} {r['Unit Price']:,.2f}", f"{curr_sym} {r['Total Material Cost']:,.2f}"])
    table_data.append(["Assembly & Wiring Labor", "Aryavarta", "1", f"{curr_sym} {labor:,.2f}", f"{curr_sym} {labor:,.2f}"])
    table_data.append(["TOTAL PRICE (Excl. Tax)", "", "", "", f"{curr_sym} {total_price:,.2f}"])
    t_bom = Table(table_data, colWidths=[220, 80, 40, 100, 100])
    t_bom.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#2B6CB0")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    story.extend([t_bom, Spacer(1, 20), Paragraph("<b>Terms:</b> 1. Taxes/Duties extra as applicable. 2. Delivery 2-3 weeks. 3. Payment 50% advance, 50% dispatch.", styles['Normal']), Spacer(1, 20), Paragraph("<b>For ARYAVARTA AUTOMATION</b><br/><br/>Authorized Signatory", styles['Normal'])])
...
def generate_fat_certificate_pdf(serial, project, client, inspector, r, y, b, hv_pass):
    ...
    story = [
        ...
        Paragraph(f"Serial: {serial} | Client: {client} | Date: {pd.Timestamp.now().strftime('%d-%b-%Y')}", styles['Normal']),
        Spacer(1, 15), Paragraph("<b>1. Insulation Resistance Test (500V Megger)</b>", styles['Heading3'])
    ]
    t_m = Table([["Phase", "Measured Value", "Min Required", "Result"], ["R-E", f"{r} MΩ", "> 2.0 MΩ", "PASS" if r>=2 else "FAIL"], ["Y-E", f"{y} MΩ", "> 2.0 MΩ", "PASS" if y>=2 else "FAIL"], ["B-E", f"{b} MΩ", "> 2.0 MΩ", "PASS" if b>=2 else "FAIL"]], colWidths=[130, 130, 140, 100])
    t_m.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#2B6CB0")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    story.extend([t_m, Spacer(1, 15), Paragraph("<b>2. HV & Safety Checks</b>", styles['Heading3'])])
    t_c = Table([["Check Item", "Standard", "Status"], ["2.5kV HV Withstand (1 Min)", "IEC 61439-1", "PASSED" if hv_pass else "FAILED"], ["Busbar Phase Clearance", "IS 8623", "PASSED"], ["Door Earth Continuity", "Safety Standard", "PASSED"]], colWidths=[240, 180, 120])
    t_c.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#4A5568")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    story.extend([t_c, Spacer(1, 25), Paragraph(f"QA Inspector: {inspector} &nbsp;&nbsp;&nbsp;&nbsp; Quality Manager Stamp: ____________", styles['Normal'])])
    doc.build(story); buf.seek(0); return buf
...
def generate_proforma_invoice_pdf(client_name, panel_type, motor_kw, total_price, curr_sym="₹"):
    ...
    story = [
        Paragraph("ARYAVARTA AUTOMATION", ParagraphStyle('H1', parent=styles['Heading1'], fontSize=18, textColor=colors.HexColor("#1A365D"))),
        Paragraph("<b>PROFORMA INVOICE</b> (HSN Code: 85371010 - Industrial Control Panels)", ParagraphStyle('H2', parent=styles['Heading2'], fontSize=11, textColor=colors.HexColor("#2B6CB0"))),
        Paragraph("Gat No. 1610, Dehu Alandi Road, Chikhali, Pune - 411062 | GSTIN: 27ABOFA4930E1ZH", styles['Normal']),
        Spacer(1, 10)
    ]
    meta = [[Paragraph(f"<b>Billed To:</b> {client_name}", styles['Normal']), Paragraph(f"<b>PI No:</b> AA/PI/{os.urandom(2).hex().upper()}", styles['Normal'])],
            [Paragraph(f"<b>Item Description:</b> {panel_type} ({motor_kw})", styles['Normal']), Paragraph(f"<b>Date:</b> {pd.Timestamp.now().strftime('%d-%b-%Y')}", styles['Normal'])]]
    t_meta = Table(meta, colWidths=[270, 270]); t_meta.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#F7FAFC")), ('BOX', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    t_inv = Table([["Particulars", "HSN Code", f"Basic ({curr_sym})", "Tax Rate", f"Tax ({curr_sym})", f"Total ({curr_sym})"],
                   [f"{panel_type} ({motor_kw}) Control Panel", "85371010", f"{curr_sym} {total_price:,.2f}", "18%", f"{curr_sym} {tax_amt:,.2f}", f"{curr_sym} {grand_tot:,.2f}"]], colWidths=[150, 65, 85, 45, 90, 105])
    t_inv.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#2B6CB0")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    bank = [["Bank Payment Details for Wire Transfer / RTGS / Swift:"], ["Bank Name: HDFC Bank Ltd | Branch: Chinchwad, Pune"], ["Account Name: ARYAVARTA AUTOMATION | Account No: 50200084920193 | IFSC: HDFC0000148 | SWIFT: HDFCINBB"]]
    t_bank = Table(bank, colWidths=[540]); t_bank.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#EDF2F7")), ('BOX', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 4)]))
    story.extend([t_meta, Spacer(1, 12), t_inv, Spacer(1, 15), t_bank, Spacer(1, 20), Paragraph("<b>For ARYAVARTA AUTOMATION</b><br/><br/>Authorized Signatory", styles['Normal'])])
    doc.build(story); buf.seek(0); return buf
...
def generate_delivery_challan_pdf(dc_no, client_name, address, vehicle_no, panel_type, gross_weight):
    ...
    story = [
        Paragraph("ARYAVARTA AUTOMATION", ParagraphStyle('H1', parent=styles['Heading1'], fontSize=18, textColor=colors.HexColor("#1A365D"))),
        Paragraph("<b>DELIVERY CHALLAN & DISPATCH NOTE</b> (For Goods Transport / Gate Pass)", ParagraphStyle('H2', parent=styles['Heading2'], fontSize=11, textColor=colors.HexColor("#2B6CB0"))),
        Paragraph("Gat No. 1610, Dehu Alandi Road, Chikhali, Pune - 411062 | GSTIN: 27ABOFA4930E1ZH", styles['Normal']),
        Spacer(1, 10)
    ]
    meta = [[Paragraph(f"<b>Challan No:</b> {dc_no}", styles['Normal']), Paragraph(f"<b>Date:</b> {pd.Timestamp.now().strftime('%d-%b-%Y')}", styles['Normal'])],
            [Paragraph(f"<b>Consignee / Client:</b> {client_name}", styles['Normal']), Paragraph(f"<b>Vehicle / LR No:</b> {vehicle_no}", styles['Normal'])],
            [Paragraph(f"<b>Delivery Address:</b> {address}", styles['Normal']), Paragraph(f"<b>HSN Code:</b> 85371010", styles['Normal'])]]
    t_meta = Table(meta, colWidths=[270, 270]); t_meta.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#F7FAFC")), ('BOX', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    t_items = Table([["S.No", "Item Description", "HSN Code", "Qty", "Gross Wt (kg)", "Status"],
                    ["1", f"{panel_type} Industrial Control Panel", "85371010", "1 Set", f"{gross_weight} kg", "Supply Only"]], colWidths=[40, 210, 80, 50, 80, 80])
    t_items.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#2B6CB0")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    story.extend([t_meta, Spacer(1, 12), t_items, Spacer(1, 20), Paragraph("<b>Declaration:</b> Material dispatched in good condition for installation & commissioning. Not for sale.", styles['Normal']), Spacer(1, 25), Paragraph("<b>Receiver's Stamp & Signature:</b> ____________________ &nbsp;&nbsp;&nbsp;&nbsp; <b>For ARYAVARTA AUTOMATION</b>", styles['Normal'])])
    doc.build(story); buf.seek(0); return buf
...
def generate_tds_pdf(client_name, panel_type, motor_kw, brand):
    ...
    story = [
        Paragraph("ARYAVARTA AUTOMATION", ParagraphStyle('H1', parent=styles['Heading1'], fontSize=18, textColor=colors.HexColor("#1A365D"))),
        Paragraph("<b>TECHNICAL DATA SHEET (TDS) & TENDER COMPLIANCE</b>", ParagraphStyle('H2', parent=styles['Heading2'], fontSize=11, textColor=colors.HexColor("#2B6CB0"))),
        Paragraph("Gat No. 1610, Dehu Alandi Road, Chikhali, Pune - 411062 | GSTIN: 27ABOFA4930E1ZH", styles['Normal']),
        Spacer(1, 12)
    ]
    data = [
    ...
    t = Table(data, colWidths=[170, 370])
    t.setStyle(TableStyle([('HEADERBACKGROUND', (0,0), (-1,0), colors.HexColor("#2B6CB0")), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#CBD5E0")), ('PADDING', (0,0), (-1,-1), 5)]))
    story.extend([t, Spacer(1, 20), Paragraph("<b>For ARYAVARTA AUTOMATION</b><br/><br/>Authorized Technical Signatory", styles['Normal'])])
    doc.build(story); buf.seek(0); return buf
